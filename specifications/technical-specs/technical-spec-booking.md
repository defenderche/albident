...
**RLS:** на `bookings` включён, разрешающих политик нет. Доступ только под `SUPABASE_SERVICE_ROLE_KEY` через Server Action — service-role обходит RLS на сервере. Anon-ключ в проекте не используется (см. `technical-spec.md §9`).
