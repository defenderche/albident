...
**RLS:** на `bookings` включён, разрешающих политик нет. Доступ только под `SUPABASE_SERVICE_ROLE_KEY` через Server Action — service-role обходит RLS на сервере. Anon-ключ для доступа к данным не используется — он применяется только для входа в админ-панель (см. `technical-spec.md §9`, `technical-spec-admin.md`).

**Просмотр заявок:** менеджер использует дашборд Supabase (Table Editor — фильтры, поиск, сортировка). Внутренней админки **для заявок** на сайте нет; админ-панель `/admin` существует только для управления услугами (см. `technical-spec-admin.md`).
