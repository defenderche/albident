...
- [x] **Фаза 5 — Чат.** Плавающая кнопка (синий круг + тень), панель с синей
      шапкой, пузыри, поле-инпут и pill-кнопки отправки/записи.
- [x] **Фаза 6 — Проверка.** `build` / `lint` / `tsc --noEmit` зелёные.
      Визуальная сверка с картой — за пользователем (`npm run dev`).
      (Lighthouse/a11y — отдельной задачей.)
